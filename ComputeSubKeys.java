...
class ComputeSubKeys {
    ...
    static int[] shiftLeftBy1(int[] data) {
        ...
        for (int i = 1; i < data.length; i++) {
            data[i - 1] = data[i]; // shift current bit left (excluding first bit)
        }
    ...
    static void outputHardwiredSchedule() {
        ...
        ks[0] = new int[48]; // empty array for key 0
        System.out.println("{},");
        ...
        for (int k = 1; k < ks.length; k++) {
            ...
            //shift by 1 for rounds 1,2,9 and 16 and by 2 for the others
            ...
            ks[k] = new int[48]; // begin subkey
            System.out.print("{");

            for (int i = 0; i < PC2.length; i++) {
                ks[k][i] = CD[PC2[i] - 1]; // P-Box compression
                
                // printing for output
                System.out.print(CD[PC2[i] - 1]);
                if (i < PC2.length - 1) {
                    System.out.print(",");
                }
            }
            // printing for output
            System.out.print("}");
            System.out.println(k < ks.length - 1 ? "," : "\n};");
        }
